...
const conflictingEmpathInfo = {
    ...
    validate: (game, _scriptPack) => {
        ...
        const empathEvents = game.events.filter((e) => e.type === 'claim_info' &&
            e.data.roleId === 'empath' &&
            e.data.claimedNumber !== undefined);
        const byNight = new Map();
        for (const event of empathEvents) {
            const existing = (_a = byNight.get(event.day)) !== null && _a !== void 0 ? _a : [];
            existing.push(event);
...
const deadPlayerNightAbility = {
    id: 'tb_dead_night_ability',
    description: '已死亡玩家声称使用夜间能力',
    validate: (game, scriptPack) => {
        void scriptPack;
        const issues = [];
        const deadPlayerIds = new Set(game.players.filter((p) => !p.isAlive).map((p) => p.id));
        const nightAbilityEvents = game.events.filter((e) => e.type === 'ability_use' && e.phase === 'night' && e.sourcePlayerId);
...
export const troubleBrewingPack = {
    id: 'trouble_brewing',
...
//# sourceMappingURL=trouble-brewing.js.map