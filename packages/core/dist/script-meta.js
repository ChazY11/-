...
export function applyScriptMeta(scriptPack) {
    const meta = scriptMetaRegistry[scriptPack.id];
    if (!meta) {
        const counts = [...scriptPack.supportedPlayerCounts].sort((a, b) => a - b);
        return Object.assign(Object.assign({}, scriptPack), { displayName: scriptPack.zhName, playerCountRange: [counts[0], counts[counts.length - 1]], nightOrder: scriptPack.roles
                .filter((role) => role.firstNight || role.otherNights)
                .sort((a, b) => { var _a, _b; return ((_a = a.nightOrder) !== null && _a !== void 0 ? _a : 999) - ((_b = b.nightOrder) !== null && _b !== void 0 ? _b : 999); })
                .map((role) => role.id) });
    }
    return Object.assign(Object.assign({}, scriptPack), { displayName: meta.displayName, playerCountRange: meta.playerCountRange, nightOrder: scriptPack.roles
            .filter((role) => role.firstNight || role.otherNights)
            .sort((a, b) => { var _a, _b; return ((_a = a.nightOrder) !== null && _a !== void 0 ? _a : 999) - ((_b = b.nightOrder) !== null && _b !== void 0 ? _b : 999); })
            .map((role) => role.id), statusTags: meta.statusTags, mechanismTags: meta.mechanismTags, storytellerControlLevers: meta.storytellerControlLevers, supportedPlayStyles: meta.supportedPlayStyles, riskWarnings: meta.riskWarnings, commonConflictTypes: meta.commonConflictTypes, storytellerGuidance: meta.storytellerGuidance });
}
export function getScriptMeta(scriptId) {
    return scriptMetaRegistry[scriptId];
}
//# sourceMappingURL=script-meta.js.map