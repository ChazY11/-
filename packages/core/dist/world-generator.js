 ...
 */
export function generateWorlds(game, scriptPack, maxResults = 50) {
...
function generateForDistribution(game, scriptPack, dist, maxResults) {
    ...
    for (let i = 0; i < attempts && worlds.length < maxResults; i++) {
        const assignment = tryGenerateAssignment(game, scriptPack, dist, townsfolk, outsiders, minions, demons);
        if (assignment) {
...
function tryGenerateAssignment(game, scriptPack, dist, townsfolk, outsiders, minions, demons) {
    ...
    if (allRoles.length !== players.length)
        return null;
...
function scoreWorld(assignments, game, scriptPack) {
    let score = 0;
    const reasons = [];
    ...
    for (const player of game.players) {
        ...
        if (assignment.roleId === player.claimedRole) {
            ...
            reasons.push(`${player.name} 声称是 ${role === null || role === void 0 ? void 0 : role.zhName}，分配一致`);
        }
    }
    ...
    if (drunkAssignment) {
        ...
        if (player === null || player === void 0 ? void 0 : player.claimedRole) {
            const claimedRoleDef = scriptPack.roles.find(r => r.id === player.claimedRole);
            if ((claimedRoleDef === null || claimedRoleDef === void 0 ? void 0 : claimedRoleDef.type) === 'townsfolk') {
                score += 5;
...
//# sourceMappingURL=world-generator.js.map