 ...
 */
export function validateGame(game, scriptPack) {
    const issues = [];
...
function checkDuplicateRoleClaims(game, scriptPack) {
    var _a;
    const issues = [];
    const claimMap = new Map(); // roleId -> playerIds
    ...
    for (const [roleId, playerIds] of claimMap.entries()) {
        if (playerIds.length > 1) {
            ...
            issues.push({
                ...
                severity: 'error',
                title: '唯一身份冲突',
                message: `多人声称自己是${(_a = role === null || role === void 0 ? void 0 : role.zhName) !== null && _a !== void 0 ? _a : roleId}：共 ${playerIds.length} 人`,
...
function checkRoleClaimCounts(game, scriptPack) {
    var _a;
    const issues = [];
    const claimedRoles = game.players
        ...
        .filter(Boolean);
    const typeCounts = { townsfolk: 0, outsider: 0, minion: 0, demon: 0, traveler: 0 };
    for (const role of claimedRoles) {
        if (role)
            typeCounts[role.type] = ((_a = typeCounts[role.type]) !== null && _a !== void 0 ? _a : 0) + 1;
    }
...
function checkEmpathConflicts(game, scriptPack) {
    void scriptPack;
    const issues = [];
    // Find all empath claim_info events
    const empathEvents = game.events.filter(e => e.type === 'claim_info' && e.sourcePlayerId);
...
function checkDeadPlayerClaims(game, _scriptPack) {
    void _scriptPack;
    const issues = [];
...
function checkPlayerCount(game, _scriptPack) {
    void _scriptPack;
    const issues = [];
...
function checkExecutionConsistency(game) {
    const issues = [];
