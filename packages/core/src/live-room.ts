...
export type NightRequestActionType =
  | 'select_one_player'
  | 'select_two_players'
  | 'select_role'
  | 'yes_no'
...
export type LiveLogEventType =
  ...
  | 'identity_sent'
  | 'night_phase_started'
  | 'night_request_created'
  | 'night_request_submitted'
  | 'night_request_resolved'
  | 'night_result_delivered'
  | 'night_phase_completed'
  | 'day_phase_started'
  | 'day_public_state_updated'
  | 'day_nomination_recorded'
  | 'day_nomination_resolved'
  | 'day_execution_recorded'
  | 'day_phase_completed'
  | 'game_finished'
  | 'phase_changed'
  | 'system';
...
export function createLiveMemberState(): RoomMemberLiveState {
  return {
    isAlive: true,
    poisoned: false,
    disabled: false,
...
export function createLiveRoom(input: CreateLiveRoomInput): LiveRoom {
  const now = Date.now();
...
export function createNightActionRequest(input: CreateNightRequestInput): NightActionRequest {
  const now = Date.now();
  return {
