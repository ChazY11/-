...
type AdviceBuilder = (game: Game, scriptPack: ScriptPack) => PerspectiveAdvice;

interface ScriptStrategyModule {
  good?: AdviceBuilder;
  evil?: AdviceBuilder;
  storyteller?: AdviceBuilder;
}

function topSuspicious(game: Game, scriptPack: ScriptPack): SuspicionScore[] {
...
function deadPlayers(game: Game) {
  return game.players.filter((player) => !player.isAlive);
}

function claimedPlayers(game: Game) {
  return game.players.filter((player) => player.claimedRole);
}

function unclaimedPlayers(game: Game) {
  return game.players.filter((player) => !player.claimedRole);
}

function statefulPlayers(game: Game) {
  return game.players.filter((player) => (
    player.state.poisoned ||
    player.state.drunk ||
    player.state.mad ||
...
function issuesWithoutInfo(game: Game, scriptPack: ScriptPack) {
  return validateGame(game, scriptPack).filter((issue) => issue.severity !== 'info');
}

function highestPressureEntry(game: Game, scriptPack: ScriptPack): SuspicionScore | null {
  return topSuspicious(game, scriptPack)[0] ?? null;
}

function highestPressureTarget(game: Game, scriptPack: ScriptPack): string {
  const target = highestPressureEntry(game, scriptPack);
  return target ? playerName(game, target.playerId) : '暂时没有特别突出的高压目标';
}

function classifyStoryState(game: Game, scriptPack: ScriptPack): '偏顺' | '偏乱' | '收束中' | '平衡中' {
  const issues = validateGame(game, scriptPack);
  const aliveCount = alivePlayers(game).length;
  if (issues.filter((issue) => issue.severity === 'error').length >= 2) return '偏乱';
  if (aliveCount <= Math.ceil(game.players.length / 2)) return '收束中';
  if (game.events.length <= game.players.length) return '偏顺';
  return '平衡中';
}

function phaseLabel(game: Game): string {
  return game.currentPhase === 'night'
    ? `第 ${Math.max(game.currentNight, 1)} 夜`
    : `第 ${Math.max(game.currentDay, 1)} 天`;
}

function openingPressure(scriptPack: ScriptPack): string {
  const info = scriptPack.storytellerControlLevers?.infoDistortion ?? 0;
  const state = scriptPack.storytellerControlLevers?.stateDisruption ?? 0;
  const revival = scriptPack.storytellerControlLevers?.resurrectionSwing ?? 0;

  if (revival >= 4 || state >= 5) return '高变动开局';
  if (info >= 4) return '高污染开局';
  if (info <= 2 && state <= 2) return '清晰开局';
  return '平衡开局';
}

function buildGoodHeadlineCards(game: Game, scriptPack: ScriptPack): StrategyHeadlineCard[] {
...
function buildGoodSections(game: Game, scriptPack: ScriptPack): StrategySection[] {
  ...
  return [
    {
      ...
      items: [
        ...
        `${scriptPack.displayName ?? scriptPack.zhName} 当前最值得从“${scriptPack.commonConflictTypes?.[0] ?? '公开信息冲突'}”开始缩圈。`,
    ...
    },
    {
      ...
      items: suspicion.length > 0
        ? suspicion.map((entry) => `${playerName(game, entry.playerId)}：当前公开视角下压力较高，原因是 ${entry.reasons[0] ?? '口径压力偏大'}。`)
...
function buildEvilHeadlineCards(game: Game, scriptPack: ScriptPack): StrategyHeadlineCard[] {
  const suspicion = topSuspicious(game, scriptPack);
  ...
  return [
    ...
    },
    {
      id: 'evil-danger',
      title: '最危险公开线',
      value: dangerLine?.title ?? '暂时没有硬冲突',
      detail: dangerLine?.message ?? '真正危险的是好人开始收束成单一可信世界线。',
      tone: 'warning',
    },
    {
      id: 'evil-target',
...
function buildEvilSections(game: Game, scriptPack: ScriptPack): StrategySection[] {
  const suspicion = topSuspicious(game, scriptPack);
  ...
  const infoRoles = game.players.filter((player) => {
    ...
    return role?.tags?.some((tag) => ['first-night', 'distance-info', 'night-info', 'count-info'].includes(tag));
  }).slice(0, 3);
  ...
  return [
    {
      ...
      items: [
        ...
        `${scriptPack.displayName ?? scriptPack.zhName} 的主要危险点在“${scriptPack.commonConflictTypes?.[0] ?? '信息链冲突'}”，邪恶方最好提前准备口径。`,
        game.currentPhase === 'day'
    ...
    },
    {
      ...
      items: dangerLines.length > 0
        ? dangerLines.map((issue) => `${issue.title ?? '危险公开线'}：${issue.message}`)
        : ['当前没有显著硬冲突，真正危险的是好人开始收束到单一可信世界线。'],
    },
  ];
}

function buildGuardrailChecks(game: Game, scriptPack: ScriptPack): string[] {
  const issues = validateGame(game, scriptPack);
  const aliveCount = alivePlayers(game).length;
...
function buildStorytellerHeadlineCards(game: Game, scriptPack: ScriptPack): StrategyHeadlineCard[] {
  ...
  return [
    ...
    },
    {
      id: 'st-lever',
      title: '说书杠杆',
      value: `信息 ${levers?.infoDistortion ?? 0} / 状态 ${levers?.stateDisruption ?? 0}`,
...
function buildStorytellerOverview(game: Game, scriptPack: ScriptPack): StrategySection {
  const issues = validateGame(game, scriptPack);
  const aliveCount = alivePlayers(game).length;
  ...
  return {
    ...
    title: '控局摘要',
    items: [
      `当前脚本：${scriptPack.displayName ?? scriptPack.zhName}。机制重心是 ${(scriptPack.mechanismTags ?? []).slice(0, 3).join(' / ') || '公开信息对抗'}。`,
      `当前阶段：${phaseLabel(game)}。存活 ${alivePlayers(game).length} 人，死亡 ${deadPlayers(game).length} 人。`,
      imbalanceHint,
      `说书杠杆：信息污染 ${scriptPack.storytellerControlLevers?.infoDistortion ?? 0} / 状态扰动 ${scriptPack.storytellerControlLevers?.stateDisruption ?? 0} / 复活摆幅 ${scriptPack.storytellerControlLevers?.resurrectionSwing ?? 0}。`,
    ],
  };
}

function buildStorytellerSections(game: Game, scriptPack: ScriptPack): StrategySection[] {
  return [
    buildStorytellerOverview(game, scriptPack),
    {
      ...
      title: '公平性护栏',
      items: [
        ...(scriptPack.riskWarnings ?? []).slice(0, 2),
        ...(scriptPack.storytellerGuidance ?? []).slice(0, 2),
...
function buildStorytellerOptions(scriptPack: ScriptPack): StrategyOption[] {
  const supported = new Set(scriptPack.supportedPlayStyles ?? ['balanced']);
  const options: StrategyOption[] = [];

  options.push(buildStyleTemplate(
    'balanced',
    scriptPack,
    '保留至少一条可读信息链，用轻度污染或轻度节奏波动制造讨论张力。',
    '如果局势已经明显偏顺，平衡推进可能显得不够有戏。',
    '教学局、混合水平局、需要稳住节奏的长桌。',
    '适合新手到中阶玩家混桌。',
    ['轻污染', '轻状态扰动', '可回溯口径'],
    ['不要同时叠加两种强干扰', '白天必须保留至少一个可追问抓手'],
  ));
  ...
  if (supported.has('high_pressure')) {
    options.push(buildStyleTemplate(
      'high_pressure',
      scriptPack,
      '把压力放在投票、提名、单点死亡或对跳上，逼出更明确的站队。',
      '若同时削弱太多信息链，容易让某一阵营直接失去决策空间。',
      '熟人局、能承受更强节奏的中高阶桌。',
      '更适合中阶以上玩家。',
      ['投票压力', '提名节奏', '单点强冲突'],
      ['不要连续两晚高压减员', '不要把同一玩家变成唯一靶点'],
    ));
  }
  ...
  if (supported.has('high_confusion')) {
    options.push(buildStyleTemplate(
      'high_confusion',
      scriptPack,
      '强化状态扰动、错口与多线并存，让玩家必须主动整理信息层次。',
      '如果没有回收线，会很快从“刺激”变成“纯乱”。',
      '熟悉脚本机制、愿意接受复杂讨论的桌。',
      '更适合熟练玩家桌。',
      ['状态扰动', '信息污染', '多焦点并存'],
      ['每晚最多强化一种主要混乱源', '第二天必须保留一条可回读主线'],
    ));
  }
  ...
  if (supported.has('high_reversal')) {
    options.push(buildStyleTemplate(
      'high_reversal',
      scriptPack,
      '围绕角色变化、阵营变化或关键误判埋伏笔，让中后段出现结构性翻盘。',
      '没有伏笔的反转会直接触发公平性争议。',
      '追求戏剧张力、但能接受前置信息铺垫的熟练局。',
      '更适合中高阶和熟练玩家。',
      ['伏笔铺设', '可解释反转', '中后段翻盘点'],
      ['反转必须能回溯', '不要连续两晚做硬翻盘'],
    ));
  ...
  if (supported.has('theatrical')) {
    options.push(buildStyleTemplate(
      'theatrical',
      scriptPack,
      '强调氛围、角色感和叙事节奏，让每轮讨论都有明确戏剧冲突。',
      '如果只剩戏剧感没有推理抓手，会让玩家觉得失控。',
      '熟人娱乐局、偏剧情和表演感的桌。',
      '适合熟人桌和愿意配合氛围的玩家。',
      ['叙事感', '公开冲突', '节奏波峰'],
      ['戏剧效果不能替代合法裁定', '每个高潮后都要留下可讨论信息'],
    ));
  }
...
function buildStorytellerOpeningSections(game: Game, scriptPack: ScriptPack): StrategySection[] {
  ...
  return [
    {
      ...
      title: '开局配置建议',
      items: [
        `推荐开局基调：${openingPressure(scriptPack)}。优先根据 ${scriptPack.displayName ?? scriptPack.zhName} 的核心机制设置第一天可读的公共话题。`,
        `人数 ${game.playerCount} 人，${playerLevelHint}，开局最好先给玩家一条能在第一天讨论的主线。`,
        scriptPack.storytellerGuidance?.[0] ?? '第一晚的操作要服务于第二天讨论，而不是只服务于夜间戏剧性。',
      ],
    },
    {
      id: 'opening-target',
...
function buildGenericAdvice(game: Game, scriptPack: ScriptPack, perspective: GameMode): PerspectiveAdvice {
  if (perspective === 'storyteller') {
    return {
      perspective,
      summary: `说书人视角会结合 ${scriptPack.displayName ?? scriptPack.zhName} 的控局杠杆、局势压力和公平性风险给出方案型建议。`,
...
function buildTroubleBrewingGood(game: Game, scriptPack: ScriptPack): PerspectiveAdvice {
  ...
  return {
    ...base,
    summary: '暗流涌动的正派辅助优先关注信息位是否被中毒/醉酒污染、外来者数量是否异常，以及对跳压力是否开始收束成单一路线。',
    headlineCards: [
      {
        id: 'tb-info-chain',
        title: '信息链健康度',
...
function buildTroubleBrewingEvil(game: Game, scriptPack: ScriptPack): PerspectiveAdvice {
  const base = buildGenericAdvice(game, scriptPack, 'evil');

  return {
    ...base,
    summary: '暗流涌动的反派辅助优先围绕“信息污染 + 对跳风险 + 外来者计数压力”来设计伪装和误导。',
    headlineCards: [
      {
        id: 'tb-evil-mask',
        title: '更稳伪装路线',
...
function buildTroubleBrewingStoryteller(game: Game, scriptPack: ScriptPack): PerspectiveAdvice {
  const base = buildGenericAdvice(game, scriptPack, 'storyteller');

  return {
    ...base,
    summary: '暗流涌动的说书重点是“清晰但不锁死”。让信息链有张力，但不能把好人一夜之间彻底送进死局。',
    sections: [
      {
        id: 'tb-st-focus',
        title: '暗流涌动控局重点',
...
function buildBadMoonRisingGood(game: Game, scriptPack: ScriptPack): PerspectiveAdvice {
  const base = buildGenericAdvice(game, scriptPack, 'good');
  const nightDeaths = game.players.filter((player) => player.state.diedAtNight).length;

  return {
    ...base,
    summary: '暗月初升的正派辅助优先看死亡节奏、保护链和复活空间，不急着把每次死人都盘成单一恶魔能力。',
    headlineCards: [
      {
        id: 'bmr-death',
        title: '死亡节奏',
...
function buildBadMoonRisingEvil(game: Game, scriptPack: ScriptPack): PerspectiveAdvice {
  const base = buildGenericAdvice(game, scriptPack, 'evil');

  return {
    ...base,
    summary: '暗月初升的反派辅助更看重死亡节奏伪装、保护链穿插和让好人误判恶魔能力方向。',
    headlineCards: [
      {
        id: 'bmr-evil-route',
        title: '更适合的推进',
...
function buildBadMoonRisingStoryteller(game: Game, scriptPack: ScriptPack): PerspectiveAdvice {
  const base = buildGenericAdvice(game, scriptPack, 'storyteller');

  return {
    ...base,
    summary: '暗月初升的说书重点是“死亡有峰值，但讨论不能断”。所有戏剧性操作都要给白天留下抓手。',
    sections: [
      {
        id: 'bmr-st-focus',
        title: '暗月初升控局重点',
...
function buildSectsAndVioletsGood(game: Game, scriptPack: ScriptPack): PerspectiveAdvice {
  const base = buildGenericAdvice(game, scriptPack, 'good');

  return {
    ...base,
    summary: '梦殒春宵的正派辅助优先处理疯狂、身份变化和阵营变化带来的“旧口径失效”，而不是把每次矛盾都当成纯狼坑。',
    headlineCards: [
      {
        id: 'sav-state',
        title: '状态扰动面',
...
function buildSectsAndVioletsEvil(game: Game, scriptPack: ScriptPack): PerspectiveAdvice {
  const base = buildGenericAdvice(game, scriptPack, 'evil');

  return {
    ...base,
    summary: '梦殒春宵的反派辅助更强调“借状态说话”。高价值误导通常来自疯狂、换角和阵营变化后的可解释错口。',
    headlineCards: [
      {
        id: 'sav-evil-line',
        title: '更适合的路线',
...
function buildSectsAndVioletsStoryteller(game: Game, scriptPack: ScriptPack): PerspectiveAdvice {
  const base = buildGenericAdvice(game, scriptPack, 'storyteller');

  return {
    ...base,
    summary: '梦殒春宵的说书重点是“高混乱但可回溯”。疯狂、换角和阵营变化都能做反转，但必须让玩家事后能理解。',
    sections: [
      {
        id: 'sav-st-focus',
        title: '梦殒春宵控局重点',
...
const scriptModules: Record<string, ScriptStrategyModule> = {
  trouble_brewing: {
...
export function getPerspectiveAdvice(game: Game, scriptPack: ScriptPack, perspective: GameMode = game.mode): PerspectiveAdvice {
  const scriptModule = scriptModules[scriptPack.id];
  const builder = scriptModule?.[perspective];
  if (builder) {
...
export function getStrategyPackSupportLevel(scriptId: string): 'full' | 'meta_only' {
  return scriptId in scriptModules ? 'full' : 'meta_only';
}

export function getStrategyPackFocus(scriptId: string): string {
  switch (scriptId) {
