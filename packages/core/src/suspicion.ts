import type { Game, ScriptPack, SuspicionScore } from './types';

/**
 * Calculate suspicion scores for all players based on events and claims.
 */
export function calculateSuspicion(game: Game, scriptPack: ScriptPack): SuspicionScore[] {
  return game.players.map(player => {
    ...
    if (!player.isAlive) {
      const wasExecuted = game.events.some(
        e => e.type === 'execution' && (e.data.diedPlayerId === player.id || e.targetPlayerId === player.id)
      );
      const diedAtNight = game.events.some(
        e => e.type === 'night_death' && (e.data.diedPlayerId === player.id || e.targetPlayerId === player.id)
      );
